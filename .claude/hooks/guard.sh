...
if [[ "$TOOL" == "Bash" ]]; then
  ...
  if echo "$CMD" | grep -qE '(^|[^A-Za-z0-9._-])\.env([^A-Za-z0-9._-]|$)'; then
    block "referencing .env via Bash is not allowed (it holds live API keys) — the Read tool already denies it; don't route around that with Bash."
  fi
fi
