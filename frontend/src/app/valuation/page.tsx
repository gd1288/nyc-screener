    ...
    <div>
      ...
      {properties && properties.length > 0 && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[240px_1fr]">
          ...
          {selected && <PropertyWorkspace key={selected.id} property={selected} onDelete={() => remove(selected.id)} />}
        </div>
      )}
    </div>
  );
...
function PropertyWorkspace({ property, onDelete }: { property: ValuationProperty; onDelete: () => void }) {
