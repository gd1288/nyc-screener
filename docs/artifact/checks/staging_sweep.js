...
// exc 0, nanCases 0, orderViolations 0, planBad 0, badLinks 0. Read the console for errors separately.
(() => {
  const tabs = ['ov', 'cf', 'st', 'as', 'pu', 'sv'], click = t => document.querySelector('.tabs button[data-t="' + t + '"]').click();
  ...
  const r = { listings: LISTINGS.rows.length, exc: 0, nanCases: 0, svgCases: 0, orderViolations: 0, planBad: 0, withBaseIrr: 0, badLinks: 0, rateChips: {} };
  for (const row of LISTINGS.rows) {
    try {
      ...
      tabTo('pu');  // purchase plan: every scenario must produce a finite plan
      ['live', 'rent', 'hack', 'then'].forEach(k => { PU.sc = k; pu(); if (!PP || !isFinite(PP.summary.cash_to_close) || !isFinite(PP.summary.net_gain_at_horizon) || PP.years.some(y => !isFinite(y.net_out))) r.planBad++; if (visible()) r.nanCases++; });
      PU = PU0();
